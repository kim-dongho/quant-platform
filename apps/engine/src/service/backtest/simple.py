...
def calculate_strategy(ticker: str, params: dict):
    ...
    bb_df = ta.bbands(
        df["close"], length=int(params.get("bb_window", 20)), std=float(params.get("bb_std", 2.0))
    )
    ...
    for i in range(len(df)):
        ...
        if (
            pd.isna(df["sma_l"].iloc[i])
            or pd.isna(df["macd_s"].iloc[i])
            or pd.isna(df["bb_u"].iloc[i])
        ):
            signals.append(0)
            continue
    ...
    return {"ticker": ticker, "results": results, "final_return": final_return}
