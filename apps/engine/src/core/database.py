...
# connectorx 는 SQLAlchemy URL 그대로 받지만 prefix `+psycopg2` 등은 빼야 함.
# 또한 connectorx 는 native PostgreSQL binary protocol 사용해 read_sql 보다
# 큰 fetch 가 5-10배 빠름. 단 named parameter 미지원이라 inline 필요.
CONNECTORX_URL = DB_URL.replace("postgresql+psycopg2://", "postgresql://")
...
def init_db():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            try:
                conn.execute(
                    text("SELECT create_hypertable('factors', 'time', if_not_exists => TRUE);")
                )
            except Exception:
                pass
